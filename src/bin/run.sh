...
trap 'echo "🛑 Señal de interrupción recibida..."; kill $(jobs -p) 2>/dev/null; echo "📊 Generando estadísticas de operación..."; echo "Archivo de log: $LOG_FILE"; echo; echo "📈 ESTADÍSTICAS DEL BOT:"; echo "========================"; bash ./src/bin/analyze_stats.sh "$LOG_FILE"; echo; echo "========================"; echo "📊 BALANCE RESUMEN:"; FINAL_BALANCE=$(get_balance); echo "💰 Balance inicial: $INITIAL_BALANCE SOL"; echo "💰 Balance final: $FINAL_BALANCE SOL"; if [ $(echo "$INITIAL_BALANCE > 0" | bc -l 2>/dev/null || echo "0") -eq 1 ]; then PROFIT_LOSS=$(echo "$FINAL_BALANCE - $INITIAL_BALANCE" | bc -l 2>/dev/null || echo "0"); PERCENTAGE_CHANGE=$(echo "scale=4; ($PROFIT_LOSS / $INITIAL_BALANCE) * 100" | bc -l 2>/dev/null || echo "0"); echo "📈 Ganancia/Pérdida: $PROFIT_LOSS SOL ($(printf '%+.2f' $PERCENTAGE_CHANGE)%)"; else echo "📈 Ganancia/Pérdida: N/A (balance inicial no disponible)"; fi; echo; echo "✅ Análisis completado"; exit 0' INT TERM
